...
class ValuationCalculator:
    ...
    def dcf_valuation(self, discount_rate: float = 10,
        ...
        try:
            ...
            if not total_shares:
                # Fallback: compute from market_cap / price
                mcap = self._safe_float(basic_info.get('market_cap'))
                price = self._safe_float(basic_info.get('latest_price'))
                if mcap and price:
    ...
    def _parse_shares(self, shares_str: str) -> Optional[float]:
        ...
        if shares_str is None or shares_str == '':
            return None
        try:
            s = str(shares_str).replace(',', '').replace('股', '')
            if '亿' in s:
                ...
                val = float(s)
                # akshare 总股本经 safe_float 后可能仅剩 亿股数值
                if 0 < val < 10000:
