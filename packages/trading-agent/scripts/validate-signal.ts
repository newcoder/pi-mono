#!/usr/bin/env node
/**
 * 快速验证任意策略信号的有效性 — 计算截面 Rank IC
 *
 * 用法:
 *   npx tsx scripts/validate-signal.ts --strategy morning_star
 *   npx tsx scripts/validate-signal.ts --strategy hammer --pool 中证500
 *   npx tsx scripts/validate-signal.ts --strategy rsi_reversal --pool 沪深300
 *
...
async function computeSignalIC(
	poolName: string,
	...
	for (const stock of stocks) {
		...
		const rawSignals = generateSignals(klines, strategy as any, {});
...
async function main() {
	...
	const strategyName = args.includes("--strategy") ? args[args.indexOf("--strategy") + 1] : "morning_star";
const poolFilter = args.includes("--pool") ? args[args.indexOf("--pool") + 1] : undefined;
console.log(`  ${strategyName} 信号有效性验证 — 截面 Rank IC`);
	console.log("═".repeat(60));
	...
	for (const poolDef of POOLS) {
		if (poolFilter && poolDef.name !== poolFilter) continue;
		let pool;
		...
		allResults[poolDef.name] = await computeSignalIC(poolDef.name, stocks, store, strategyName);
	}

	// ─── Summary table ───────────────────────────────────────────

	console.log(`\n${"═".repeat(90)}`);
	console.log(`  汇总: ${strategyName} 截面 Rank IC`);
