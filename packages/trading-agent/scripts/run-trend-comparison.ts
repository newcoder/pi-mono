...
async function main() {
	...
	for (const pool of POOLS) {
		for (const strategy of STRATEGIES) {
			const t0 = Date.now();
			console.log(`Running ${pool.name} (${pool.id}) + ${strategy} ...`);
			try {
				const res = await backtestStrategyTool.execute(`bt-trend-${pool.id}-${strategy}`, {
					pool_id: pool.id,
			...
			fs.writeFileSync("backtest_trend_comparison.json", JSON.stringify(results, null, 2));
		}
	}

	console.log("\nAll done. Results saved to backtest_trend_comparison.json");
	store.close();
}
