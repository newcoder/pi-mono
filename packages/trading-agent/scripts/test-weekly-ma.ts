...
async function main() {
	...
	for (const strategy of ["supertrend", "volume_contraction"] as const) {
		for (const rankBy of ["ma_alignment", "weekly_ma_alignment"] as const) {
			...
			console.log(
				`${strategy} + ${rankBy}: return ${result.metrics.totalReturn.toFixed(2)}%, sharpe ${result.metrics.sharpeRatio.toFixed(3)}, drawdown ${result.metrics.maxDrawdown.toFixed(2)}%, winRate ${result.metrics.winRate.toFixed(2)}%, trades ${sellCount}, avgHold ${result.metrics.avgHoldingDays.toFixed(2)}`,
			);
		}
	}
