...
const VARIANTS = [
	...
	{ label: "fresh_high", params: { requireFreshHigh: 1 } },
] as const;
...
async function main() {
	...
	for (const pool of POOLS) {
		for (const variant of VARIANTS) {
			...
			fs.writeFileSync("backtest_volume_contraction_filter.json", JSON.stringify(results, null, 2));
		}
	}

	console.log("\nAll done. Results saved to backtest_volume_contraction_filter.json");
	store.close();
}
