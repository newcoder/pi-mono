...
async function main() {
	...
	for (const sizing of SIZING_METHODS) {
		for (const tc of TEST_CASES) {
			const t0 = Date.now();
			console.log(`Running pool53 ${MODE} + ${sizing} + ${tc.rankBy} + ${tc.strategy} ...`);
			try {
				const res = await backtestStrategyTool.execute(`bt-pool53-${MODE}-${sizing}-${tc.rankBy}-${tc.strategy}`, {
					pool_id: 53,
			...
			fs.writeFileSync("backtest_pool53.json", JSON.stringify(results, null, 2));
		}
	}

	console.log("\nAll done. Results saved to backtest_pool53.json");
	store.close();
}
