...
async function main() {
	...
	if (!pool) throw new Error("Pool 80 not found");
	...
	const lastDateTrades = result.trades.filter((t) => t.date === "2026-06-26" && t.code === "000539");
	console.log(`Trades for 000539 on 2026-06-26: ${lastDateTrades.length}`);
	for (const t of lastDateTrades) {
		console.log(t);
	}

	// Also show all buy trades on last day to see memo context
	console.log("\nAll buy trades on 2026-06-26:");
	for (const t of result.trades.filter((t) => t.date === "2026-06-26" && t.direction === "buy")) {
		console.log(`${t.code} ${t.direction} ${t.shares}股 @${t.price.toFixed(2)} memo=${t.memo}`);
	}
