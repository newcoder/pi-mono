	...
	for (const mode of MODES) {
		for (const rankBy of RANK_BYS) {
			for (const strategy of STRATEGIES) {
				const t0 = Date.now();
				console.log(`Running ${mode} + ${rankBy} + ${strategy} ...`);
				try {
					const res = await backtestStrategyTool.execute(`bt-${mode}-${rankBy}-${strategy}`, {
						pool_id: 110,
					...
					results.push({ mode, rankBy, strategy, elapsedMs: Date.now() - t0, error: (e as Error).message });
					console.error(`  -> error:`, (e as Error).message);
				}
				fs.writeFileSync("backtest_rankby_sw2_max10.json", JSON.stringify(results, null, 2));
			}
		}
	}

	console.log("\nAll done. Results saved to backtest_rankby_sw2_max10.json");
	store.close();
}
