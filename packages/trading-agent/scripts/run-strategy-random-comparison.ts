...
async function main() {
	...
	const summaryPath = join(reportsDir, `strategy-comparison-random-${RANDOM_RUNS}runs-${pool.name}-${START}-${END}.csv`);
	const summaryStream = createWriteStream(summaryPath, { encoding: "utf-8" });
	summaryStream.write(
		"strategy,rank_by,random_runs,total_return_pct,annualized_return_pct,sharpe_ratio,max_drawdown_pct,win_rate_pct,profit_factor,avg_holding_days,total_trades,report_url\n",
	);
	...
	for (const strategy of STRATEGIES) {
		...
		console.log(`\n[${strategy} / random x${RANDOM_RUNS}] running...`);
		...
		try {
			...
			summaryStream.write(`${strategy},random,${RANDOM_RUNS},ERROR,ERROR,ERROR,ERROR,ERROR,ERROR,ERROR,ERROR,\n`);
			continue;
		}
		...
		try {
			const genResult = await generatePoolBacktestReport(
				{
					title: `${pool.name} ${strategy}(random×${RANDOM_RUNS}) 批量回测报告`,
					poolName: pool.name,
					strategy,
