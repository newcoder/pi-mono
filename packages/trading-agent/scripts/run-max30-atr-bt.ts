...
const TEST_CASES = [
	...
	{ strategy: "volume_contraction", rankBy: "ma_alignment" },
...
async function main() {
	...
	for (const sizing of SIZING_METHODS) {
		for (const tc of TEST_CASES) {
			const t0 = Date.now();
			console.log(`Running max30 ${MODE} + ${sizing} + ${tc.rankBy} + ${tc.strategy} ...`);
			try {
				const res = await backtestStrategyTool.execute(`bt-max30-${MODE}-${sizing}-${tc.rankBy}-${tc.strategy}`, {
					pool_id: 110,
			...
			fs.writeFileSync("backtest_max30_atr_sw2.json", JSON.stringify(results, null, 2));
		}
	}

	console.log("\nAll done. Results saved to backtest_max30_atr_sw2.json");
	store.close();
}
