...
async function main() {
	...
	for (const mode of MODES) {
		for (const strategy of STRATEGIES) {
			const t0 = Date.now();
			console.log(`Running hs300 ${mode} ${RANK_BY} ${SIZING} ${strategy} ...`);
			try {
				const res = await backtestStrategyTool.execute(`bt-hs300-${mode}-${RANK_BY}-${SIZING}-${strategy}`, {
					pool_id: POOL_ID,
			...
			fs.writeFileSync("backtest_hs300_ma_atr.json", JSON.stringify(results, null, 2));
		}
	}

	console.log("\nAll done. Results saved to backtest_hs300_ma_atr.json");
	store.close();
}
